...
const COLLAPSE_KEY = 'sparkorbit.admin.sidebar-collapsed';
...
const displayName = computed(() => auth.user?.displayName || auth.user?.username || '管理员');
...
<template>
  ...
  >
    ...
    >
      <div
        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-t-accent/80 to-t-accent2/70 text-white shadow-lg"
        aria-hidden="true"
      >
        <svg viewBox="0 0 16 16" class="h-4.5 w-4.5" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="8" cy="8" r="2.4" />
          <path d="M13.8 8c0 1-2.6 2.1-5.8 2.1S2.2 9 2.2 8 4.8 5.9 8 5.9 13.8 7 13.8 8Z" transform="rotate(-24 8 8)" />
        </svg>
      </div>
      <div class="min-w-0 flex-1" :class="collapsed ? 'lg:hidden' : ''">
        <h1 class="truncate text-[15px] font-semibold text-t-1">系统控制台</h1>
        <p class="t-kicker mt-0.5">SparkOrbit Ops</p>
      </div>
      <button
        type="button"
    ...
    <!-- 分组导航 -->
    <nav ref="navRef" class="relative flex-1 overflow-y-auto overflow-x-hidden px-3 py-3" aria-label="管理端导航">
    ...
    <div class="border-t border-t-line/10 p-3">
      ...
      >
        <OrbAvatar palette="violet" :size="32" :label="displayName" />
        <div class="min-w-0 flex-1" :class="collapsed ? 'lg:hidden' : ''">
          <p class="truncate text-[13px] font-medium text-t-1">{{ displayName }}</p>
          <p class="truncate text-[11px] text-t-3">管理员账号</p>
        </div>
        <button
          type="button"
